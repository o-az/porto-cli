...
impl RelayServer {
    pub async fn new() -> Result<Self> {
        ...
        let listener = tokio::net::TcpListener::bind(addr)
            .await
            .map_err(|e| PortoError::AccountCreation(format!("Failed to bind listener: {e}")))?;
        let port = listener
            .local_addr()
            .map_err(|e| PortoError::AccountCreation(format!("Failed to get local address: {e}")))?
            .port();
    ...
    pub async fn send_message(&self, topic: &str, payload: serde_json::Value) -> Result<String> {
        ...
        self.state
            .sender
            .send(message)
            .map_err(|e| PortoError::AccountCreation(format!("Failed to send message: {e}")))?;
        ...
        tokio::select! {
            ...
            result = rx.recv() => {
                match result {
                    ...
                    None => Err(PortoError::AccountCreation("Channel closed".to_string())),
                }
            }
            ...
            _ = async {
                while let Ok(msg) = receiver.recv().await {
                    if msg.topic == "rpc-response"
                        && let Some(id) = msg.payload.get("id").and_then(|v| v.as_u64())
                            && id == request_id
                                && let Some(result) = msg.payload.get("result") {
                                    let mut pending = self.state.pending_responses.write().await;
                ...
                Err(PortoError::AccountCreation("Broadcast channel closed".to_string()))
            }
            // Timeout after 5 minutes
            _ = tokio::time::sleep(Duration::from_secs(300)) => {
                Err(PortoError::AccountCreation("Request timed out".to_string()))
            }
        }
    }
}
...
async fn post_handler(
    ...
    if let (Some(topic), Some(payload)) = (
        ...
        if topic == "rpc-response"
            && let Some(id) = payload.get("id").and_then(|v| v.as_u64())
            && let Some(result) = payload.get("result")
        {
...
async fn keys_handler(State(state): State<RelayState>) -> Json<serde_json::Value> {
